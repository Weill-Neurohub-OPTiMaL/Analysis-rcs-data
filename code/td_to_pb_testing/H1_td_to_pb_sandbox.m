...
pb5 = combinedDataTable.Power_Band5(pb_mask);
...
ylim([0,1e4])
ylabel({'Measured PB from TD', '[RC+S units]'})
yyaxis right
plot((timestamps_fft - zero_time)/1000, pb1_est)
ylim([0,1e4])
ylabel({'Computed PB from TD', '[RC+S units]'})
xlabel('Time [sec]')
...
% extract the power band
data_pb = rcs_fft_to_pb(data_fft, L, fs_td, hann_win, bit_shift);
pb5_est = sum(data_pb(:,bins(5,1):bins(5,2)), 2);
...
ylabel({'Computed PB from TD', '[RC+S units]'})
xlabel('Time [sec]')
title({['Power band 5 bin indices [',num2str(bins(5,:)),']'], ...
...
writerObj = VideoWriter('its_working.mp4', 'MPEG-4');
